...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/libvirt_events/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/libvirt_events/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/libvirt_events/*"
            )
            ...
            #    "--include=src/saltext/libvirt_events/*,tests/*"
            # )
        finally:
